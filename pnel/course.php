				...
				<div class="panel-body no-padding">
					<table class="table table-striped">
						...
						include_once('courses.php');
														$ses = new courses;
	  $ff = $ses ->RUNSearch("select  * from courses where staff_username = '".$_SESSION['stfs']."' ");
								   
								   while($rows = mysqli_fetch_assoc($ff)){
	   					?>
							<tr>
                            <form method="post">
		<td><input type="text" readonly hidden name="user" value="<?php echo $rows['staff_username']; ?>"/></td>
        <td><input type="text" readonly hidden name="cors<?php echo $rows['courses_name']; ?>" value="<?php echo $rows['courses_name']; ?>"/></td>
                                
	<td><a href="lecture.php?<?php echo $rows['courses_name']; ?>"><?php echo $rows['courses_name']; ?></a></td>
								<td><?php echo $rows['courses_index']; ?></td>
				<td><input type="submit" name="crs<?php echo $rows['courses_name']; ?>" value="delete course"/></td>
                    ...
                    <table border="2">
                     <form method="post" >
						...
						?>
                                <th>
                                <input type="text" name="newyear"  readonly value="first" placeholder="course name">
                                </th>
								<th>
                                <input type="text" name="newstf"  readonly value="<?php echo $_SESSION['stf']; ?>" />
                                </th>
<th><input type="text" name="newcrs" value="<?php echo isset($_POST['newcrs']) ? $_POST['newcrs'] : '' ?>"
 placeholder="course name"></th>
								<th><input type="text" name="newindex" value="" placeholder="course index"></th>
                             ...
                             <?php } ?>   
								...
								if(isset($_POST['addcrs'])){
									...
									$addcrs->setcourses_index($_POST['newindex']);
									$addcrs->setcourses_name($_POST['newcrs']);
									$addcrs->setyear_name($_POST['newyear']);
									$msg =  $addcrs->Add();	
